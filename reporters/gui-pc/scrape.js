...
let reportSource = "pc"; // PC 패키지 고정: 항상 G_PC
...
function setScheduleState(theaterId, rows){
  ...
  state.schedules[theaterId]=(rows||[]).map(r=>({...r,movieTitle:decodeHtml(r.movieTitle),hallName:decodeHtml(r.hallName),imax:imaxKeys.has(scheduleKey(r)) || /imax|아이맥스/i.test(`${r.hallName} ${r.movieTitle}`)}));
}
...
function fmtShowDate(playDate){
  ...
  let m,d;
  if(/^\d{4}-\d{2}-\d{2}/.test(s)){ m=Number(s.slice(5,7)); d=Number(s.slice(8,10)); }
  else if(/^\d{8}/.test(s)){ m=Number(s.slice(4,6)); d=Number(s.slice(6,8)); }
  else return s;
  if(!m||!d) return s;
  return m+"/"+d;
}
...
function render(){
  ...
  for(const id of showIds){
    ...
    const bodyHtml=list.length?list.map(r=>{const n=Number(r.restSeats),cls=n===0?'zero':n<=5?'low':'good',status=n===0?'매진':n<=5?'잔여 적음':'잔여';const imax=r.imax||/imax|아이맥스/i.test((r.hallName||'')+' '+(r.movieTitle||''));return '<div class="showRow '+cls+'"><div class="showTop"><b>'+esc((r.playDate?fmtShowDate(r.playDate)+' ':'')+(r.startTime||''))+'</b><span class="'+(imax?'imax':'muted')+'">'+(imax?'IMAX':esc(decodeHtml(r.hallName||'')))+'</span></div><div class="showTitle">'+esc(decodeHtml(r.movieTitle||''))+'</div><div class="showBottom"><b>'+esc(r.restSeats)+' / '+esc(r.totalSeats)+'석</b><span>'+status+' · '+esc(fmtShowDate(r.playDate||''))+'</span></div></div>';}).join(''):'<div class="muted" style="padding:16px;text-align:center;font-size:12px">회차 없음</div>';
...
function serverDashboard(){
  ...
  const boardHtml=showIds.map(id=>{
    ...
    const list=(byTheater.get(id)||[]).slice(0,800);
    const body=list.length?list.map(r=>{const n=Number(r.restSeats),cls=n===0?"zero":n<=5?"low":"good",status=n===0?"매진":n<=5?"잔여 적음":"잔여";const imax=r.imax||/imax|아이맥스/i.test(`${r.hallName||""} ${r.movieTitle||""}`);return `<div class="showRow ${cls}"><div class="showTop"><b>${htmlEsc((r.playDate?fmtShowDate(r.playDate)+" ":"")+(r.startTime||""))}</b><span class="${imax?'imax':'muted'}">${imax?'IMAX':htmlEsc(decodeHtml(r.hallName||""))}</span></div><div class="showTitle">${htmlEsc(decodeHtml(r.movieTitle||""))}</div><div class="showBottom"><b>${htmlEsc(r.restSeats)} / ${htmlEsc(r.totalSeats)}석</b><span>${status} · ${htmlEsc(fmtShowDate(r.playDate||""))}</span></div></div>`}).join(""):'<div class="muted" style="padding:16px;text-align:center;font-size:12px">회차 없음</div>';
...
if(!TOKEN){console.error("[오류] 서버 인증 토큰이 없습니다. config.env 의 NAS_WORKER_TOKEN 값을 확인하세요.");process.exit(1);}
