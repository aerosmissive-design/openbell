...
type TestItem = {
  ...
  body: string;
  bookingUrl: string;
  theaterId: (typeof THEATERS)[number];
  movieTitle: string;
  playDate: string;
  startTime: string;
  hallName: string;
...
export const sendReservationTest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const scan = await runScan({
      daysAhead: 7,
      theaters: [...THEATERS],
      gasWebUrl: data.gasWebUrl?.trim() || undefined,
      sources: { official: true, naver: true, gas: Boolean(data.gasWebUrl?.trim()) },
    });
    const now = kstNow();
    ...
    for (const theaterId of THEATERS) {
      ...
      selected.push({
        ...
        bookingUrl: alertBookingUrl(show),
        theaterId,
    ...
    if (selected.length !== THEATERS.length) {
      const missing = THEATERS.filter((id) => !selected.some((x) => x.theaterId === id));
      throw new Error(`실제 상영 회차를 찾지 못했습니다: ${missing.join(", ")}. 잠시 뒤 다시 눌러 주세요.`);
    }

    const items = selected.map((x, i) => ({
      ...x,
      id: `alert:test-reservation:${x.theaterId}:${Date.now()}:${i}`,
