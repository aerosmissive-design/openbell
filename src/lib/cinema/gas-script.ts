...
function grokStatus_() {
  ...
  return {
    ...
    stamp: (typeof SCRIPT_STAMP !== "undefined" ? SCRIPT_STAMP : ""),
    reporters: reporterSummary_(),
    showcacheN: (function () {
      try {
        var sc = loadShowcache_();
        if (Array.isArray(sc)) return sc.length;
        if (sc && sc.rows) return sc.rows.length;
      } catch (e) {}
      return 0;
    })()
  };
...
function normalizeReportSource_(source) {
  ...
  if (s === "nas423" || s === "g-nas423+" || s === "g_nas423+" || s === "g_ds423" || s === "g-ds423" || s === "g-ds423+" || s === "g_ds423+" || s === "ds423" || s === "ds423+") return "nas423";
  if (s === "nas225" || s === "g-nas225+" || s === "g_nas225+" || s === "g_ds225" || s === "g-ds225" || s === "g-ds225+" || s === "g_ds225+" || s === "ds225" || s === "ds225+") return "nas225";
...
function seatSourceLabel_(source) {
  if (source === "nas423") return "g-ds423+";
  if (source === "nas225") return "g-ds225+";
  return "g-pc";
}
...
function saveReporterPayload_(props, sk, payloadObj) {
  var rows = payloadObj.rows || [];
  var lines = [];
  ...
  var c;
  for (c = 0; c < n; c++) {
    props.setProperty(sk + ":c" + c, blob.slice(c * chunk, (c + 1) * chunk));
  }
  for (c = n; c < 24; c++) {
    try { props.deleteProperty(sk + ":c" + c); } catch (eDel) {}
  }
}

function loadReporterPayload_(props, sk) {
  var raw = props.getProperty(sk) || "";
  if (!raw) return null;
  var parsed = null;
  try { parsed = JSON.parse(raw); } catch (e0) { return null; }
  if (!parsed) return null;
  if (Array.isArray(parsed.rows)) return parsed;
  if (!parsed.compact) return parsed;
  var n = Number(parsed.chunks) || 0;
  var parts = [];
  for (var c = 0; c < n; c++) parts.push(props.getProperty(sk + ":c" + c) || "");
  var blob = parts.join("");
  var rows = [];
  if (blob) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      rows.push({
        ...
        movieTitle: cols[3] || "",
        restSeats: rest,
        totalSeats: isFinite(tot) ? tot : undefined,
        movieNo: cols[6] || "",
        scnsNo: cols[7] || "",
        scnSseq: cols[8] || ""
      });
...
function handleSeatReport_(body) {
  ...
  if (merge) {
    var byKey = {};
    var prev = loadReporterPayload_(PropertiesService.getScriptProperties(), sk);
  ...
  var payloadObj = { at: Date.now(), rows: rows, mode: merge ? "merge" : "full", source: source };
  var props = PropertiesService.getScriptProperties();
  try {
    saveReporterPayload_(props, sk, payloadObj);
    try { props.setProperty(reportLegacyKey_(theaterId), JSON.stringify({ at: payloadObj.at, source: source, n: rows.length })); } catch (eLeg) {}
  } catch (eSave) {
    return jsonOut_({ ok: false, error: "store " + String(eSave && eSave.message ? eSave.message : eSave) });
  }
...
function loadReporterLive_() {
  ...
  theaters.forEach(function (theaterId) {
    ...
    sources.forEach(function (src) {
      var parsed = loadReporterPayload_(props, reportSourceKey_(src, theaterId));
...
function reporterSummary_() {
  var props = PropertiesService.getScriptProperties();
  var theaters = CONFIG.theaters || ["cgv_yongsan", "cgv_yeongdeungpo", "megabox_coex", "megabox_namyangju"];
  var sources = ["pc", "nas423", "nas225"];
  var out = [];
  var now = Date.now();
  theaters.forEach(function (tid) {
    sources.forEach(function (src) {
      var p = loadReporterPayload_(props, reportSourceKey_(src, tid));
      if (!p) return;
      try {
        var at = Number(p.at) || 0;
        var n = Array.isArray(p.rows) ? p.rows.length : Number(p.n) || 0;
        out.push({
          theaterId: tid,
          source: src,
          label: seatSourceBoardLabel_(src),
          at: at,
...
function gasBoardHtml_() {
  ...
  for (i = 0; i < limit; i++) {
    ...
    rowsHtml += "<tr><td>" + escapeHtml_(r.theater || r.theaterId)
      ...
      + "</td><td>" + escapeHtml_(seatSourceBoardLabel_(r.seatSource || "scrape"))
      + "</td></tr>";
  }
  if (!rowsHtml) {
