...
import type { RankingMovie, ScanResult, ScanSources, Showtime, TheaterId, TheaterScan } from "./types";
...
export async function runScan(input: {
  ...
  const emptyCatalog = { ranking: [] as RankingMovie[], showing: [] as RankingMovie[], catalog: [] as RankingMovie[] };
...
function byDateForTheater(shows: Showtime[], theaterId: TheaterId): Map<string, Showtime[]> {
  ...
  for (const show of shows) {
    ...
    const normalized = normalizePlayDate(show.playDate);
    const list = map.get(normalized) ?? [];
    list.push({ ...show, playDate: normalized });
...
export async function pingSeatmap(input: { url?: string; fresh?: boolean; theaterId?: TheaterId; daysAhead?: number }) {
  ...
  if (input.url?.trim() && ((wantMega && !megaHit) || (wantCgv && !cgvOfficialHit))) {
...
async function loadGasTimetable(url: string, days: number, theaterId?: TheaterId): Promise<Showtime[]> {
  const params: Record<string, string> = { op: "live", days: String(Math.min(days, 30)) };
...
async function loadGasJson(url: string, params: Record<string, string>): Promise<Showtime[]> {
  let target: URL;
  try { target = new URL(url.trim()); } catch { return []; }
  const host = target.hostname;
  if (!host.endsWith("script.google.com") && !host.endsWith("googleusercontent.com")) return [];
  for (const [key, value] of Object.entries(params)) target.searchParams.set(key, value);
  try {
    const res = await fetch(target.toString(), { redirect: "follow", signal: AbortSignal.timeout(25000) });
    ...
    return json.map(gasRowToShowtime).filter((row): row is Showtime => Boolean(row));
  } catch { return []; }
}
...
function gasRowToShowtime(row: GasShowRow): Showtime | null {
  ...
  const playDate = normalizePlayDate(String(row.date || ""));
...
async function loadGasSeatmap(url: string, fresh = false): Promise<{ status: "ok" | "old" | "denied" | "empty" | "badurl" | "timeout"; map: GasSeatMap }> {
  let target: URL;
  try { target = new URL(url.trim()); } catch { return { status: "badurl", map: {} }; }
  const host = target.hostname;
  if (!host.endsWith("script.google.com") && !host.endsWith("googleusercontent.com")) return { status: "badurl", map: {} };
  target.searchParams.set("op", "seatmap");
  if (fresh) target.searchParams.set("fresh", "1");
  try {
    const res = await fetch(target.toString(), { redirect: "follow", signal: AbortSignal.timeout(fresh ? 20000 : 15000) });
    const text = await res.text();
    if (!text || /sign in|accounts\.google/i.test(text)) return { status: "denied", map: {} };
    const start = text.indexOf("{"); const end = text.lastIndexOf("}");
    if (start < 0 || end <= start) return { status: "old", map: {} };
    const json = JSON.parse(text.slice(start, end + 1)) as GasSeatMap;
    if (!json || typeof json !== "object") return { status: "old", map: {} };
    const cgvCount = Object.keys(json).filter((key) => key.startsWith("k:") || key.startsWith("cgv:")).length;
    if (!Object.keys(json).length) return { status: "empty", map: {} };
    if (fresh && !cgvCount) return { status: "empty", map: json };
    return { status: "ok", map: json };
  } catch (err) {
