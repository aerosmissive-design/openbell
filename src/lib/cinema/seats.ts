...
function seatLookupKeys(row: Showtime): string[] { const titleKey = normalizeTitle(row.movieTitle); const siteNo = SITE_NO[row.theaterId] ?? ""; const prefix = row.chain === "cgv" ? "k:" : "m:"; const keys = [row.id]; const rowHalls = hallVariants(row.hallName); for (const clock of clockVariants(row.playDate, row.startTime)) { const stem = siteNo ? `${prefix}${siteNo}|${clock.playDate}|${clock.startTime}` : ""; if (!stem) continue; for (const hall of rowHalls) keys.push(`${stem}|${hall}`); if (!rowHalls.length) { if (titleKey) keys.push(`${stem}|${titleKey}`); if (row.movieNo) keys.push(`${stem}|${row.movieNo}`); } } return [...new Set(keys.filter(Boolean))]; }
