...
function hallVariants(hall: string): string[] { const base = normHall(hall); const stripped = base.replace(/\[.*?\]/g, ""); const loose = stripped.replace(/관$/, ""); const titled = normalizeTitle(hall); return [...new Set([base, stripped, loose, `${loose}관`, titled].filter(Boolean))]; }
function clockVariants(playDate: string, startTime: string): { playDate: string; startTime: string }[] { const time = parseClock(startTime); if (!time) return []; const out = [{ playDate, startTime: time.hhmm }]; if (time.hour >= 24 && playDate.length === 8) { out.push({ playDate: addYmd(playDate, 1), startTime: `${String(time.hour - 24).padStart(2, "0")}:${time.minute}` }); out.push({ playDate, startTime: `${time.hour}:${time.minute}` }); } if (time.hour < 4 && playDate.length === 8) out.push({ playDate: addYmd(playDate, -1), startTime: `${time.hour + 24}:${time.minute}` }); return out; }
function parseClock(raw: string): { hour: number; minute: string; hhmm: string } | null { const digits = String(raw || "").replace(/\D/g, ""); if (digits.length < 3) return null; const padded = digits.length === 3 ? `0${digits}` : digits.slice(0, 4); const hour = Number(padded.slice(0, padded.length - 2)); const minute = padded.slice(-2); if (!Number.isFinite(hour) || Number(minute) > 59) return null; const hhmm = `${String(hour % 24).padStart(2, "0")}:${minute}`; return { hour, minute, hhmm: hour >= 24 ? `${hour}:${minute}` : hhmm }; }
function addYmd(ymd: string, days: number) { const y = Number(ymd.slice(0, 4)); const m = Number(ymd.slice(4, 6)); const d = Number(ymd.slice(6, 8)); const dt = new Date(Date.UTC(y, m - 1, d + days)); return `${dt.getUTCFullYear()}${String(dt.getUTCMonth() + 1).padStart(2, "0")}${String(dt.getUTCMonth() + 1).padStart(2, "0")}`; }
