...
export function describeSeatPing(result: {
  ...
  if (result.status === "badurl") return { ok: false, text: "웹앱 주소가 올바르지 않습니다. /exec 로 끝나는 주소를 붙여넣으세요." };
...
export function mergeShowtimes(primary: Showtime[], extra: Showtime[] = []): Showtime[] {
  ...
  const keyOf = (row: Showtime) => `${row.theaterId}|${row.playDate}|${normTime(row.startTime)}|${normalizeTitle(row.movieTitle)}|${normalizeTitle(row.hallName)}`;
  ...
  for (const row of primary) {
    ...
    map.set(key, { ...prev, ...row, restSeats: row.restSeats ?? prev.restSeats, totalSeats: row.totalSeats ?? prev.totalSeats, bookingUrl: row.bookingUrl || prev.bookingUrl, movieNo: row.movieNo || prev.movieNo, seatLive: row.restSeats != null ? (row.seatLive ?? true) : prev.seatLive, seatCheckedAt: row.seatCheckedAt ?? prev.seatCheckedAt });
  }
  ...
  for (const row of primary) {
    ...
    const candidates = (loose.get(looseKey) ?? []).filter((k) => {
      const candidate = map.get(k);
      return Boolean(candidate && !candidate.bookingUrl && hallsMatch(row.hallName, candidate.hallName));
    });
...
export function applyCgvSeatHits(rows: Showtime[], map: SeatHitMap, overwrite = false, asLive = true): Showtime[] {
  const nowIso = new Date().toISOString();
  const stampKeep = (row: Showtime): Showtime => {
    if (typeof row.restSeats !== "number" || !Number.isFinite(row.restSeats)) return { ...row, seatLive: false };
    return { ...row, seatLive: row.seatLive ?? true, seatCheckedAt: row.seatCheckedAt ?? nowIso };
  };
  const keys = Object.keys(map);
  if (!keys.length) return rows.map(stampKeep);
  return rows.map((row) => {
    const had = typeof row.restSeats === "number" && Number.isFinite(row.restSeats);
    if (had && !overwrite) return stampKeep(row);
    const hit = lookupSeatHit(row, map);
    if (!hit) return stampKeep(row);
    return { ...row, restSeats: hit.rest, totalSeats: hit.total ?? row.totalSeats, seatLive: asLive, seatCheckedAt: asLive ? nowIso : hit.at ? new Date(hit.at).toISOString() : row.seatCheckedAt ?? null };
  });
}

export function indexSeatHit(map: SeatHitMap, row: { theaterId: string; playDate: string; startTime: string; movieTitle?: string; hallName?: string; movieNo?: string; chain?: "cgv" | "megabox" }, rec: SeatHit) {
  const chain = row.chain ?? (String(row.theaterId).startsWith("cgv") ? "cgv" : "megabox");
  for (const key of seatLookupKeys({ id: "", theaterId: row.theaterId as Showtime["theaterId"], theaterName: "", chain, movieTitle: row.movieTitle || "", movieNo: row.movieNo || "", playDate: row.playDate, startTime: row.startTime, endTime: null, hallName: row.hallName || "", formats: [], restSeats: rec.rest, totalSeats: rec.total, bookingUrl: "", bookable: true })) {
...
export function putSeatHit(map: SeatHitMap, row: Showtime) {
  ...
  indexSeatHit(map, row, { rest: row.restSeats, total: row.totalSeats, at: Date.now() });
}

function packHit(hit: SeatHit): SeatHit { return { rest: hit.rest, total: hit.total ?? null, at: hit.at }; }

export function lookupSeatHit(row: Showtime, map: SeatHitMap): SeatHit | null {
  // 같은 영화·시간이어도 관이 다르면 절대 다른 관의 잔여석을 붙이지 않는다.
  for (const key of seatLookupKeys(row)) {
    const hit = map[key];
    if (hit && typeof hit.rest === "number" && Number.isFinite(hit.rest)) return packHit(hit);
  }
  ...
  for (const clock of clockVariants(row.playDate, row.startTime)) {
    ...
    const halls = hallVariants(row.hallName);
    if (halls.length) {
      for (const hall of halls) {
        const hit = map[`${prefix}${hall}`];
        if (hit && typeof hit.rest === "number" && Number.isFinite(hit.rest)) return packHit(hit);
      }
      // 관이 있는 회차는 제목/movieNo만으로 다른 관의 숫자를 붙이지 않는다.
      continue;
    }
    for (const fallback of [normalizeTitle(row.movieTitle), row.movieNo].filter(Boolean)) {
      const hit = map[`${prefix}${fallback}`];
      if (hit && typeof hit.rest === "number" && Number.isFinite(hit.rest)) return packHit(hit);
    }
  }
  return null;
...
export function seatFreshnessLabel(show: Pick<Showtime, "restSeats" | "seatLive" | "seatCheckedAt">): string | null {
  if (typeof show.restSeats !== "number" || !Number.isFinite(show.restSeats)) return null;
...
export function summarizeSeatDelta(before: Showtime[], after: Showtime[]) {
  ...
  return { shows: gains.length, seats: gains.reduce((sum, row) => sum + row.added, 0), lines: gains.map((row) => `• ${formatShowPlace(row.show)}에서 ${row.added}석이 추가됐습니다`) };
}
...
export function formatShowPlace(show: { theaterName: string; chain: "megabox" | "cgv" | string; playDate: string; startTime: string; hallName?: string | null }) {
  const chain = show.chain === "cgv" ? "CGV" : "메가박스";
  const name = String(show.theaterName || "").replace(/^메가박스\s*/, "").replace(/^CGV\s*/, "").trim();
  return [`${name} ${chain}`.trim(), formatPlayDate(show.playDate), formatClock(show.startTime), show.hallName].filter(Boolean).join(" ");
}
...
function clockVariants(playDate: string, startTime: string): { playDate: string; startTime: string }[] {
  ...
  if (time.hour >= 24 && playDate.length === 8) {
    out.push({ playDate: addYmd(playDate, 1), startTime: `${String(time.hour - 24).padStart(2, "0")}:${time.minute}` });
    out.push({ playDate, startTime: `${time.hour}:${time.minute}` });
  }
  if (time.hour < 4 && playDate.length === 8) out.push({ playDate: addYmd(playDate, -1), startTime: `${time.hour + 24}:${time.minute}` });
  return out;
}
...
function addYmd(ymd: string, days: number) { const y = Number(ymd.slice(0, 4)); const m = Number(ymd.slice(4, 6)); const d = Number(ymd.slice(6, 8)); const dt = new Date(Date.UTC(y, m - 1, d + days)); return `${dt.getUTCFullYear()}${String(dt.getUTCMonth() + 1).padStart(2, "0")}${String(dt.getUTCDate()).padStart(2, "0")}`; }
function normTime(time: string) { const parsed = parseClock(time); return parsed?.hhmm ?? String(time || "").trim(); }
function normHall(hall: string) { return String(hall || "").toUpperCase().replace(/[\s|]+/g, ""); }
function hallsMatch(a: string, b: string) { const aa = hallVariants(a); const bb = hallVariants(b); if (!aa.length || !bb.length) return false; return aa.some((v) => bb.includes(v)); }

export type SeatChange = { itemId: string; show: Showtime; prev: number; next: number; delta: number };

export function diffStarSeats(queue: BookingIntent[], shows: Showtime[]): { nextQueue: BookingIntent[]; changes: SeatChange[] } {
  const byId = new Map(shows.map((s) => [s.id, s]));
  const changes: SeatChange[] = [];
  const nextQueue = queue.map((item) => {
    const show = byId.get(item.showtimeId) ?? shows.find((s) => s.theaterId === item.theaterId && s.playDate === item.playDate && s.startTime === item.startTime && normalizeTitle(s.hallName) === normalizeTitle(item.hallName)) ?? shows.find((s) => s.theaterId === item.theaterId && titlesMatch(s.movieTitle, item.movieTitle) && clockVariants(s.playDate, s.startTime).some((c) => clockVariants(item.playDate, item.startTime).some((q) => q.playDate === c.playDate && q.startTime === c.startTime && hallsMatch(s.hallName, item.hallName))));
    if (!show || show.restSeats == null || show.seatLive === false) return item;
    const prev = item.restSeats;
    if (typeof prev === "number" && prev !== show.restSeats) changes.push({ itemId: item.id, show, prev, next: show.restSeats, delta: show.restSeats - prev });
    if (prev !== show.restSeats || item.totalSeats !== show.totalSeats) return { ...item, restSeats: show.restSeats, totalSeats: show.totalSeats, bookingUrl: show.bookingUrl || item.bookingUrl };
    return item;
  });
...
export function notifyCopy(items: AlertItem[], opts?: { total?: number }): { subject: string; text: string; telegramHtml: string } {
  ...
  const subject = allSeats ? `[오픈벨] 잔여석 변동 ${total}건` : allOpens ? `[오픈벨] 예매 오픈 ${total}건` : `[오픈벨] 알림 ${total}건`;
  const text = `${subject}\n\n${items.map((a) => { const link = a.bookingUrl ? `\n바로 예매 ${a.bookingUrl}` : ""; return `${a.title}\n${a.body}${link}`; }).join("\n\n")}`;
  const telegramHtml = `${escapeHtml(subject)}\n\n${items.map((a) => { const href = escapeAttr(a.bookingUrl); const link = href ? `\n<a href="${href}">바로 예매</a>` : ""; return `<b>${escapeHtml(a.title)}</b>\n${escapeHtml(a.body)}${link}`; }).join("\n\n")}`;
  return { subject, text, telegramHtml };
}

export function tweetCopy(items: AlertItem[]): string { const rows = items.slice(0, 3); const lines = ["홀드현알리미"]; for (const item of rows) { lines.push(item.title); if (item.body) lines.push(item.body); if (item.bookingUrl) lines.push(item.bookingUrl); } if (items.length > rows.length) lines.push(`외 ${items.length - rows.length}건`); const text = lines.filter(Boolean).join("\n"); return text.length > 270 ? `${text.slice(0, 267)}…` : text; }
export function escapeHtml(value: string) { return String(value || "").replace(/&/g, "&" + "amp;").replace(/</g, "&" + "lt;").replace(/>/g, "&" + "gt;"); }
export function escapeAttr(value: string) { return String(value || "").replace(/&/g, "&" + "amp;").replace(/"/g, "&" + "quot;").replace(/</g, "&" + "lt;"); }
