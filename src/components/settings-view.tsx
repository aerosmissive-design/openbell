...
import { exchangeKakaoCode, peekTelegramChat, sendAlertEmail, sendGasTest, sendTelegram } from "@/lib/cinema/scan";
...
export function SettingsView({ lastScan }: { lastScan: ScanResult | null }) {
  ...
  async function sendReservationChannelTest(channel: "mail" | "telegram" | "kakao") {
    if (channel === "mail" && !mailEnabled(config) && !config.gasWebUrl.trim()) {
      toast.error("먼저 메일을 연결하세요.");
      return;
    }
    setSendingTest(true);
    try {
      ...
      toast.success(`${result.count}개 극장 예매 알림 테스트를 보냈습니다.`);
    } catch (err) {
        ...
        >
          ...
          {user ? (
            <>
              ...
              <Button variant="outline" className="mt-2 w-full" disabled={sendingTest} onClick={() => void sendReservationChannelTest("mail")}>
                메일 예매 알림 테스트
              </Button>
            </>
          ) : (
