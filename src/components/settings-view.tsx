...
function channelShot(result?: string, at?: number) {
...
function channelWorkLine(key: "mail" | "telegram" | "kakao", health: NotifyHealth) {
  return [
    `베셀 ${channelShot(health.vercel.lastNotify?.[key], health.vercel.lastNotify?.at)}`,
    `스크립트 ${health.gasNotify ? channelShot(health.gasNotify[key], health.gasNotify.at) : health.gasOk ? "대기" : "확인 못 함"}`,
  ].join(" · ");
}
