...
export function SettingsView({ lastScan }: { lastScan: ScanResult | null }) {
  ...
  async function sendReservationChannelTest(channel: "mail" | "telegram" | "kakao") {
    ...
    try {
      ...
      const names = result.theaters
        .map((row) => `${row.movieTitle} ${row.playDate.slice(4, 6)}.${row.playDate.slice(6, 8)} ${row.startTime}`)
        .join(" · ");
  ...
  async function sendTestMail() {
    ...
    try {
      ...
      toast.success(
        result.needsConfirm
          ? "첫 메일은 확인 링크입니다. 받은편지함에서 한 번만 눌러 주세요."
          : `${email}로 보냈습니다. 받은편지함을 확인하세요.`,
      );
    } catch (err) {
    ...
    <div className="flex flex-col gap-6">
          ...
          {showGasSection ? (
            <>
              <p className="mt-2 text-sm leading-relaxed text-muted">
                베셀 서버에 문제가 있을 때도 알림을 받고 싶으면 예비로 켭니다. 극장·잔여석 조회는 구글스크립트 없이 돌아갑니다.
              </p>
      ...
      <section className="rounded-xl bg-surface p-4 shadow-border">
        ...
        <div className="mt-4 border-t border-border pt-3">
          <button
            ...
            className="flex min-h-11 w-full items-center justify-between gap-3 text-left"
          >
            <h2 className="text-xs font-medium tracking-[0.16em] text-muted">상영시간 및 잔여석 현황 출처</h2>
            <span className="shrink-0 text-xs text-muted">{showStatus ? "접기" : "펼치기"}</span>
          </button>
          ...
          {showStatus ? (
            <>
              <p className="mt-2 text-sm leading-relaxed text-muted">
                상영시간과 잔여석 현황은 각각 실제 조회에 사용된 출처와 시각을 표시합니다. 잔여석은 출처의 우선순위가 아니라 같은 회차에서 가장 최근 성공 조회값을 현재값으로 사용합니다.
              </p>
              <div className="mt-4">
                  ...
                  {THEATERS.map((theater) => {
                    ...
                    return (
                      <Fragment key={theater.id}>
                        <p className="text-muted">{theater.name}</p>
              ...
              <div className="mt-5">
                ...
                <p className="mt-1 text-muted">
                  각 출처에서 마지막으로 성공적으로 들어온 조회시각입니다. 극장별 가장 최근 시각은 빨간색으로 표시합니다.
                </p>
                  ...
                  <table className="w-full min-w-[900px] text-sm">
                    ...
                    <tbody>
                      {THEATERS.map((theater) => {
                        ...
                        const latestAt = Math.max(0, ...entries.map((e) => e.at));
                        return (
                            ...
                            {entries.map((entry) => (
                              <td
                                ...
                                className={cn(
                                  ...
                                  entry.at > 0 && entry.at === latestAt ? "text-danger font-medium" : "",
                                )}
                              >
      ...
      <section className="rounded-xl bg-surface p-4 shadow-border">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xs font-medium tracking-[0.16em] text-muted">4열 전광판</h2>
          <a
            href="https://openbell-fawn.vercel.app/board"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex min-h-10 items-center gap-1.5 rounded-md bg-bg px-3 text-xs text-muted ring-1 ring-border"
          >
            전광판 보기
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted">
          로컬 OpenBell Reporter가 보내는 전광판 데이터를 큰 화면에서 4열로 확인합니다.
        </p>
      </section>
      <section className="rounded-xl bg-surface p-4 shadow-border">
        <h2 className="text-xs font-medium tracking-[0.16em] text-muted">알림 설정</h2>
        <h3 className="mt-4 text-xs font-medium tracking-[0.16em] text-muted">며칠 뒤까지</h3>
        <p className="mt-2 text-xs leading-relaxed text-faint">
          오늘부터 며칠 뒤 상영까지 볼지입니다. 극장이 아직 안 연 날짜는 비어 있습니다.
        </p>
          ...
          {[5, 7, 10, 15, 30].map((n) => (
            <button
              ...
              className={cn(
                ...
                config.daysAhead === n ? "bg-pick text-fg ring-1 ring-border-strong" : "bg-bg text-muted",
              )}
            >
              {n}일
            </button>
        ...
        <h3 className="mt-5 text-xs font-medium tracking-[0.16em] text-muted">앱을 꺼도 알림</h3>
        <p className="mt-2 text-sm leading-relaxed text-muted">
        ...
        <div className="mt-4 space-y-3">
          <div className="rounded-lg bg-bg p-3 ring-1 ring-border">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-fg">메일</p>
                <p className="mt-1 text-xs text-muted">{mailEnabled(config) ? (loginEmail || config.email) : "꺼짐"}</p>
              </div>
              <Switch
                checked={config.emailNotify}
                onCheckedChange={(on) => {
                  if (on && !(loginEmail || config.email.trim())) {
                    toast.error("먼저 로그인해 주세요.");
                    return;
                  }
                  setConfig({ emailNotify: on, ...(on ? { email: loginEmail || config.email } : {}) });
                }}
                label={config.emailNotify ? "켜짐" : "꺼짐"}
              />
            </div>
            <Button
              variant="outline"
          ...
          <div className="rounded-lg bg-bg p-3 ring-1 ring-border">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-fg">카톡</p>
                <p className="mt-1 text-xs text-muted">{config.kakaoRefreshToken ? "연결됨" : "꺼짐"}</p>
              </div>
              <span className="text-xs text-muted">{config.kakaoRefreshToken ? "연결됨" : "연결 필요"}</span>
            </div>
            <Button
              variant="outline"
              className="mt-3 w-full"
          ...
          <div className="rounded-lg bg-bg p-3 ring-1 ring-border">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-fg">텔레그램</p>
                <p className="mt-1 text-xs text-muted">{config.telegramToken && config.telegramChatId ? "연결됨" : "꺼짐"}</p>
              </div>
              <span className="text-xs text-muted">{config.telegramToken && config.telegramChatId ? "연결됨" : "연결 필요"}</span>
            </div>
            <Button
              variant="outline"
              className="mt-3 w-full"
      ...
      <section className="rounded-xl bg-surface p-4 shadow-border">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted">라이트, 다크, 또는 기기 설정을 따릅니다.</p>
        <div className="mt-3 grid grid-cols-3 gap-2">
    ...
    >
      ...
      >
        <div className="min-w-0">
          ...
          {detail ? <p className="mt-0.5 text-[10px] leading-4 text-muted">{detail}</p> : null}
        </div>
    ...
    <div>
      ...
      {user ? (
        <div className="mt-2 space-y-2 text-sm leading-relaxed text-muted">
          ...
          {user.primaryEmail && user.displayName ? <p>{user.primaryEmail}</p> : null}
          <p>
            로그인하면 별표·메일·텔레그램·카톡이 이 계정에 저장됩니다. 베셀 서버가 알림을 보냅니다. 구글스크립트는
            예비이며 없어도 됩니다.
          </p>
          {authEnabled ? (
            <Button
...
function AlertPathStatus({ health }: { health: NotifyHealth }) {
  const n = Number(health.vercel.alive || health.vercel.reachable) + Number(health.gasAlive);
  const summary =
    n >= 2
  ...
  return (
    <div className="mt-3 rounded-lg bg-bg px-3 py-2.5 text-fg ring-1 ring-border">
      ...
      <div className="mt-2 grid grid-cols-[5.2rem_minmax(0,0.9fr)_minmax(0,0.9fr)_minmax(0,1.4fr)] gap-x-2 gap-y-1 text-[10px] leading-4 text-fg">
        ...
        <span className="font-medium text-fg">베셀</span>
        <span>{watchLine(health.vercel)}</span>
        <span>{queryLine(health.vercel)}</span>
        <span>{githubWakeLine(health.vercel)}</span>
        <span className="font-medium text-fg">구글스크립트(예비)</span>
        <span>{health.gasAlive ? "켜짐" : health.gasOk ? "웹앱 응답" : "확인 못 함"}</span>
        <span>
...
function githubWakeLine(probe: AliveProbe) {
  if (!probe.reachable) return "확인 못 함";
  if (probe.githubWakeAlive) return "깃허브 깨움";
  return "대기";
}
...
function relayOutageLine(watch: CgvRelayHealth) {
  const names =
    watch.theaters.map((id) => THEATERS.find((row) => row.id === id)?.shortName).filter(Boolean).join("·") ||
    "용산·영등포";
