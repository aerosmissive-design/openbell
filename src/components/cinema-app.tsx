...
export function CinemaApp() {
  ...
  const fastQuery = useQuery({
    queryKey: ["scan-fast", enabledTheaters, config.daysAhead, config.gasWebUrl],
  ...
  const fullQuery = useQuery({
    queryKey: ["scan-full", enabledTheaters, config.daysAhead, config.gasWebUrl],
    enabled: enabledTheaters.length > 0,
    queryFn: () =>
      scanCinema({
        data: {
  ...
  const viewScan = useMemo(() => {
    ...
    const theaters = (scan?.theaters ?? []).map((t) => {
      ...
      const cachedOnly =
        hasSeats &&
        showtimes.every((row) => row.restSeats == null || row.seatLive === false);
  ...
  useEffect(() => {
    ...
    const items = fresh.map((show) =>
      toAlert(show, (alertScan.theaters ?? []).flatMap((t) => t.showtimes)),
    );
    pushAlerts(items);
    ...
    <div className="mx-auto flex min-h-dvh max-w-lg flex-col bg-bg md:max-w-5xl">
      ...
      <header className="sticky top-0 z-20 border-b border-border bg-bg px-5 pb-3 pt-[max(1rem,env(safe-area-inset-top))]">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-[11px] font-medium tracking-[0.18em] text-muted">특별관 예매 알람</p>
...
function mergeSeatSourceTimes(
  base?: ScanResult["seatSourceTimes"],
  extra?: ScanResult["seatSourceTimes"],
): ScanResult["seatSourceTimes"] {
  const out: NonNullable<ScanResult["seatSourceTimes"]> = { ...(base ?? {}) };
  for (const [theaterId, times] of Object.entries(extra ?? {})) {
    const current = { ...(out[theaterId] ?? {}) };
    for (const [key, value] of Object.entries(times ?? {})) {
      if (!current[key] || new Date(value).getTime() >= new Date(current[key]).getTime())
        current[key] = value;
    }
    out[theaterId] = current;
  }
  return out;
}
...
function mergeScanResults(fast: ScanResult | null, full: ScanResult | null): ScanResult | null {
  ...
  return {
    ...
    catalog: mergeMovieCatalog(
      ...
      ranking,
      showing,
      moviesFromShowtimes(theaters.flatMap((theater) => theater.showtimes)),
    ),
    theaters,
  };
}
