...
export class CgvBookingAgent {
  ...
  async autoSelectSeats(count: number, preference: Omit<SeatPreference, "count"> = {}) {
    const page = this.getPage();
    const seatCandidates = page.locator(SEAT_MAP_SELECTOR);
  ...
  private async readSeatMap(page: Page): Promise<SeatPoint[]> {
    const raw = await page.locator(SEAT_MAP_SELECTOR).evaluateAll((elements) =>
      elements
        .map((el) => {
  ...
  private async findSeat(page: Page, seatId: string) {
    ...
    const byRole = page.getByRole("checkbox", { name: new RegExp(`^${escapeRegExp(seatId)}$`, "i") }).first();
    if (await byRole.count()) return byRole;
    const byButton = page.getByRole("button", { name: new RegExp(`^${escapeRegExp(seatId)}$`, "i") }).first();
