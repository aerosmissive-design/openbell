...
- Shallow CGV booking URLs no longer abort. Title/date/time clicks. MegaBox refused. Payment URLs refused.

### v2.0.15 notes

- Login overlay on the booking page is treated as `LOGIN_REQUIRED`. Hold-wait prints every 30s.

### v2.0.14 notes

- `4-save-login.cmd` refuses to save unless logout / MY CGV is visible.

### v2.0.13 notes

- Missing `cgv-storage.json` no longer crashes launch. Browser locale `ko-KR`.
